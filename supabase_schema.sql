...
ALTER TABLE IF EXISTS public.tithes ADD COLUMN IF NOT EXISTS archived BOOLEAN DEFAULT false;
ALTER TABLE IF EXISTS public.tithes ADD COLUMN IF NOT EXISTS "balanceGroup" TEXT;
ALTER TABLE IF EXISTS public.offerings ADD COLUMN IF NOT EXISTS description TEXT;
ALTER TABLE IF EXISTS public.offerings ADD COLUMN IF NOT EXISTS type TEXT;
ALTER TABLE IF EXISTS public.projects ADD COLUMN IF NOT EXISTS status TEXT;
ALTER TABLE IF EXISTS public.votes ADD COLUMN IF NOT EXISTS "voterName" TEXT;

-- 2.5 CORREGIR RESTRICCIONES (NOT NULL) ANTIGUAS QUE BLOQUEAN INSERCIONES
DO $$
BEGIN
  BEGIN
    ALTER TABLE public.projects ALTER COLUMN "startDate" DROP NOT NULL;
  EXCEPTION
    WHEN undefined_column THEN null;
  END;
  
  BEGIN
    ALTER TABLE public.votes ALTER COLUMN "memberName" DROP NOT NULL;
  EXCEPTION
    WHEN undefined_column THEN null;
  END;
END $$;
